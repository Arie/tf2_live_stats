...
liveStats.controller('DamageController', ['$scope', '$timeout', '$http', function($scope, $timeout, $http) {

  $scope.Math = window.Math;

  var get_stats = function(last_or_current) {
    if ($scope.matchId != undefined) { 
      $http.get('/damage/' + last_or_current + '/' + $scope.matchId + '?delay=' + $scope.timeDelay + '&' + new Date().getTime()).then(function(response) {
