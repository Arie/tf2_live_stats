Capistrano::Configuration.instance.load do
  ...
  namespace :puma do
    ...
    task :start, :roles => lambda { fetch(:puma_role) }, :on_no_matching_servers => :continue do
      puma_env = "production"
      run "cd #{current_path} && #{fetch(:bundle_cmd, "bundle")} exec puma -e #{puma_env} -b 'tcp://127.0.0.1:3020' -t 2:16 -S #{shared_path}/sockets/puma.state --control 'unix://#{shared_path}/sockets/pumactl.sock' >> #{shared_path}/log/puma.log 2>&1 &", :pty => false
    end
